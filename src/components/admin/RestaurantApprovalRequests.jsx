...
function RestaurantApprovalRequests() {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosAdminInstance.post('/restaurant/approval-requests', {
        page,
        size,
      ...
      if (response.status === 200 && response.data) {
        ...
        setError('Failed to fetch restaurant approval requests');
      }
    } catch (err) {
      console.error('Error fetching restaurant approval requests:', err);
  ...
  const handleViewDetails = (restaurantRegistrationId) => {
    navigate(`/admin/restaurant/request/${restaurantRegistrationId}`);
  };
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="mb-4 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">Restaurant Approval Requests</h1>
        ...
        <div className="flex flex-col md:flex-row gap-3 md:items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder="Restaurant or owner email"
      ...
      <div className="overflow-hidden rounded-xl border border-gray-800 bg-gradient-to-b from-black/70 to-black/60 backdrop-blur">
          ...
          <table className="min-w-full text-sm">
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-900/70 text-white">
                ...
                <th className="py-3.5 px-4 text-left font-semibold border-b border-gray-800">Owner Name</th>
                <th className="py-3.5 px-4 text-left font-semibold border-b border-gray-800">Owner Email</th>
            ...
            <tbody className="divide-y divide-gray-800">
              {isLoading && showSkeleton
                ? renderSkeletonRows()
                : restaurantRequests.map((request, idx) => (
                    <tr
                      key={request.restaurantRegistrationId}
                      className={`${idx % 2 === 0 ? 'bg-black/40' : 'bg-black/30'} text-white hover:bg-gray-800/50 transition-colors`}
                    >
                      <td className="py-3 px-4 font-medium">{request.restaurantName}</td>
                      <td className="py-3 px-4 text-gray-200">{request.ownerName}</td>
                      <td className="py-3 px-4 text-gray-300">{request.ownerEmail}</td>
                      <td className="py-3 px-4">
                        <span
                      ...
                      <td className="py-3 px-4">
                        <Button
                          onClick={() => handleViewDetails(request.restaurantId)}
        ...
        {/* Empty state */}
        {!isLoading && restaurantRequests.length === 0 && (
          <div className="p-10 text-center text-gray-300">No restaurant approval requests found.</div>
        )}
...
export default RestaurantApprovalRequests;
